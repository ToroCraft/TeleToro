...
public class TeletoryChunkProviderFromEnd implements IChunkGenerator {
	/** RNG. */
	private final Random rand;

	protected static final IBlockState END_STONE = Blocks.WOOL.getDefaultState();
	...
	public TeletoryChunkProviderFromEnd(World worldObjIn, long seed) {
	 ...
	 */
	public void setBlocksInChunk(int xChunk, int zChunk, ChunkPrimer primer) {
		// int i = 2;
		// int j = 3;
		// int k = height;
		// int l = 3;
		...
		buffer = this.getHeights(buffer, xChunk * 2, 0, zChunk * 2, 3, height, 3);

		for (int xOuterLoop = 0; xOuterLoop < 2; ++xOuterLoop) {
			for (int zOuterLoop = 0; zOuterLoop < 2; ++zOuterLoop) {
				for (int yOuterLoop = 0; yOuterLoop < 32; ++yOuterLoop) {

					double d0 = 0.25D;
					double d1 = this.buffer[(xOuterLoop * 3 + zOuterLoop) * height + yOuterLoop + 0];
					double d2 = this.buffer[(xOuterLoop * 3 + zOuterLoop + 1) * height + yOuterLoop + 0];

					double d3 = this.buffer[((xOuterLoop + 1) * 3 + zOuterLoop + 0) * height + yOuterLoop + 0];
					double d4 = this.buffer[((xOuterLoop + 1) * 3 + zOuterLoop + 1) * height + yOuterLoop + 0];

					double d5 = (this.buffer[((xOuterLoop + 0) * 3 + zOuterLoop + 0) * height + yOuterLoop + 1] - d1) * 0.25D;
					double d6 = (this.buffer[((xOuterLoop + 0) * 3 + zOuterLoop + 1) * height + yOuterLoop + 1] - d2) * 0.25D;
					double d7 = (this.buffer[((xOuterLoop + 1) * 3 + zOuterLoop + 0) * height + yOuterLoop + 1] - d3) * 0.25D;
					double d8 = (this.buffer[((xOuterLoop + 1) * 3 + zOuterLoop + 1) * height + yOuterLoop + 1] - d4) * 0.25D;
					...
					for (int yLoop = 0; yLoop < 4; ++yLoop) {
						...
						for (int xLoop = 0; xLoop < 8; ++xLoop) {
							...
							for (int zLoop = 0; zLoop < 8; ++zLoop) {
								...
								int x = xLoop + xOuterLoop * 8;
								int y = yLoop + yOuterLoop * 4;
								int z = zLoop + zOuterLoop * 8;
	...
	public Chunk provideChunk(int x, int z) {
		...
		// this.worldObj.getBiomeProvider().getBiomes(this.biomesForGeneration,
		// x * 16, z * 16, 16, 16);
	...
	public void populate(int x, int z) {
		 ...
		 * this.endCityGen.generateStructure(this.worldObj, this.rand, new //
		 * ChunkPos(x, z)); // }
		 * 
		 * this.worldObj.getBiome(blockpos.add(16, 0,
		 * 16)).decorate(this.worldObj, this.worldObj.rand, blockpos); long i =
		 * (long) x * (long) x + (long) z * (long) z;
		 * 
		 * if (i > 4096L) { float f = this.getIslandHeightValue(x, z, 1, 1);
		 * 
		 ...
		 * this.rand.nextInt(16) + 8)); } }
		 * 
		 * if (this.getIslandHeightValue(x, z, 1, 1) > 40.0F) { int j =
		 * this.rand.nextInt(5);
		 * 
		 * for (int k = 0; k < j; ++k) { int l = this.rand.nextInt(16) + 8; int
		 * i1 = this.rand.nextInt(16) + 8; int j1 =
		 * this.worldObj.getHeight(blockpos.add(l, 0, i1)).getY();
		 * 
		 * if (j1 > 0) { int k1 = j1 - 1;
		 * 
		 * if (this.worldObj.isAirBlock(blockpos.add(l, k1 + 1, i1)) &&
		 * this.worldObj.getBlockState(blockpos.add(l, k1, i1)).getBlock() ==
		 * Blocks.END_STONE) { BlockChorusFlower.generatePlant(this.worldObj,
		 * blockpos.add(l, k1 + 1, i1), this.rand, 8); } } } } }
		 * 
		 * 
		 */
